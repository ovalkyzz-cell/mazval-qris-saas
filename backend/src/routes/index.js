...
router.get('/setup/migrate', async (req, res) => {
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS transactions (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), provider_transaction_id VARCHAR(100), amount INTEGER NOT NULL, total_amount INTEGER, description TEXT, qr_url TEXT, payment_url TEXT, status VARCHAR(20) DEFAULT 'pending', plan_id INTEGER, discount_amount INTEGER DEFAULT 0, expired_at TIMESTAMP, paid_at TIMESTAMP, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())`);
    await pool.query(`ALTER TABLE transactions ADD COLUMN IF NOT EXISTS promo_code_id INTEGER`);
    await pool.query(`ALTER TABLE transactions ADD COLUMN IF NOT EXISTS plan_id INTEGER`);
